...
io.on("connection", (socket) => {
  ...
  socket.on("webrtc_offer", (event) => {
    ...
    socket.broadcast.to(event.roomId).emit("webrtc_offer", {sdp:event.sdp, sendID:socket.id});
    console.log('\n');
  });
  socket.on("webrtc_answer", (event) => {
    ...
    socket.broadcast.to(event.roomId).emit("webrtc_answer", {sdp:event.sdp, sendID:socket.id});
    console.log('\n');
  });
  socket.on("webrtc_ice_candidate", (event) => {
    ...
    socket.broadcast.to(event.roomId).emit("webrtc_ice_candidate",  {event, sendID:socket.id});
    console.log('\n');
  });
});
